#!/usr/bin/env python3
"""Adaptive retrieval depth on FEVER.

Mirrors scripts/17_adaptive_retrieval.py but evaluates on FEVER. The three
strategies (FIXED-3, FIXED-10, ADAPTIVE) and metrics are identical so the
results are directly comparable to SciFact.

Run:
    python scripts/18_adaptive_retrieval_fever.py
    python scripts/18_adaptive_retrieval_fever.py --max-dev-claims 1000
"""
...
def main():
    parser = argparse.ArgumentParser(description="Adaptive retrieval depth on FEVER")
    parser.add_argument("--max-dev-claims", type=int, default=500)
    ...
    console.print("\n[bold blue]Adaptive Retrieval — FEVER[/bold blue]\n")
    ...
    console.print(f"\nRunning on {len(fv.dev_claims)} dev claims...\n")

    per_claim = []
    for claim in tqdm(fv.dev_claims, desc="Pipeline"):
        ...
        v3, r3 = joint_predict(joint, claim.text, ranked_3, fv.abstracts)
        v10, r10 = joint_predict(joint, claim.text, ranked_10, fv.abstracts)
    ...
    sweep_table = Table(title="Adaptive — Threshold Sweep (FEVER)")
    ...
    cmp_table = Table(title="Strategy Comparison (FEVER dev)")
    ...
    out_path = results_dir / "18_adaptive_retrieval_fever.json"
